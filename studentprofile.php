...
if (isset($_POST['register'])) {
	...
	$studentphoto = $_FILES['profile_imge'];
	$studentphotoName = $studentphoto['name'];
	$studentphotoTmpName = $studentphoto['tmp_name'];

	$studentphotoDirectory = "upload/student/";

	// Check if a new image is provided
	if (!empty($studentphotoName)) {
		// Get the existing staff record to unlink the old image
		$existingStaffRecord = mysqli_query($con, "SELECT profile_imge FROM student WHERE studentid = $studentid");
		$existingImage = mysqli_fetch_assoc($existingStaffRecord);

		if ($existingImage && file_exists($existingImage['profile_imge'])) {
			// Unlink the old image
			unlink($existingImage['profile_imge']);
		}
		// Generate a unique name for the new image
		$uniquestudentphotoName = uniqid() . '_' . $studentphotoName;
		$studentphotoPath = $studentphotoDirectory . $uniquestudentphotoName;

		// Move the new image
		if (move_uploaded_file($studentphotoTmpName, $studentphotoPath)) {
		} else {
	...
	if ($_FILES["profile_imge"]["name"] != "") {
		$qry = $qry . ", profile_imge='$studentphotoPath'";
	}
	...
	if (mysqli_query($con, $qry)) {
		echo "<script>alert('Profile updated successfully...!!');window.location='studentprofile.php';</script>";
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						...
						<img src="<?php echo $rsstudentprofile['profile_imge']; ?>" style="width: 150px;height: 170px;">
					</div>
				</div>

...
if (isset($_GET['id'])) {
  ...
  // Create the unique ID by combining "user" with the existing ID
  $uniqueID = "allumni" . $userId;

  // Update the record with the unique ID
  $qry = "UPDATE tbluser SET status='Active', reg_date=CURDATE(), uniqueID='$uniqueID' WHERE userid=" . $userId;
